...
function mountPhone({ stage, node, state, assets, go, notify }) {
  ...
  const hint = el('div', 'phone-hint', '↑↓ 选择 · 空格 确认 · M/Esc 主页 · D 删除');
  ...
  const APPS = [
    { key: 'mail', label: '邮件', badge: () => mailIds.filter(id => !flags.phone.read.includes(id)).length },
    { key: 'contacts', label: '通讯录', badge: null },
    { key: 'album', label: '相册', badge: null }
  ];
  const tabLabel = () => (APPS.find(a => a.key === tab) || {}).label || '';
  ...
  function renderHome() {
    ...
    APPS.forEach((app, i) => {
      ...
      row.append(icon, el('div', 'home-label', app.label));
  ...
  function render() {
    ...
    const homeBtn = button('◂ 主页', () => goHome());
  ...
  function renderMailList() {
    ...
    mailIds.forEach((id, i) => {
      ...
      row.append(from, el('div', 'r-time', m.time), el('div', 'r-sub', m.subject || '（无主题）'));
