...
更新：2026-09-06。范围：`game/` 的游玩舞台、素材替换约定、必要的流程修复与验证。本文为后续 AI / 人工接手入口；后续改代码时同步更新「实现状态」「验证记录」。根目录登录、开场视频、开始菜单不在本次重做范围。
...
当前随附 SVG 是代码绘制的布局示意素材，不是正式角色设计，不代表最终美术质量。未拷贝参考游戏的立绘、背景、配音或代码文件。窗口结构已改好，最终视觉表现仍取决于后续提供的图片。
...
game/index.html                      脚本加载顺序、stage、菜单、状态提示
  data/assets.js                     资源 ID → 路径 + 缺图兜底
  data/story/prologue.js             42 个序章节点、对白、分支、演出字段
  ...
  assets/placeholders/               随附、实际存在的 SVG 示意图
...
当前 images 全部指向实际存在的占位 SVG；清单右侧中文注释说明各 ID 的用途。把一张占位图的公共文件直接覆盖会影响所有引用它的 ID，因此正式素材应放进 images，并分别改清单。程序不扫描文件夹，不会根据新文件名自动匹配剧情。

| 素材 ID | 后续应提供 |
| --- | --- |
| bg-apartment-dusk / bg-apartment-night | 出租屋黄昏 / 深夜 |
| bg-university / bg-empty-apartment / bg-hallway | 大学、空公寓、走廊；部分仅登记尚未使用 |
| bg-tunnel | 隧道横版背景；背景目前固定，玩家摄像机滚动 |
| bg-coast-night / bg-coast-blue | 月夜海岸 / 蓝光海岸；现阶段共享海岸示意图 |
| bg-room-white | 白底演出页；节点文字提及黑屏，最终需编剧确认统一 |
| portrait-kio / portrait-kio-young | 成年 / 年少基生 |
| portrait-airi / portrait-airi-flash | 爱理 / 闪回；当前人物共用中性轮廓示意图 |
| kio-walk | 步行角色单图；尚无精灵帧动画 |
| photo-seaside / parcel-label / bankbook 等 UI ID | 各道具独立图片；目前共用照片示意图 |
...
缺图兜底：`sceneImage()` 图片加载失败时切到 `fallbacks` 对应 SVG；兜底也失败则隐藏图像，舞台保留基础底色，不显示破图图标。此机制用于 scene.js 场景层。手机相册仍沿用原有图像显示逻辑，步行角色加载失败回到几何人物；不要把这一点理解成所有模块均有统一图像诊断。
...
当前主线是：出租屋旧邮件 → 主管来电 → 回忆 / 通讯录删除 → 错送快递 → 爱理回信 → 扔手机或出门（汇合）→ 隧道步行 → 海岸邮件隐藏链接 → 返回分支结局或蓝光主线终幕。**格子探索和弹幕模块在仓库中，但当前 42 节点序章不经过它们**，不能宣称它们已接入当前主线。

存档键为 `ily-save-v1:<账号>`，每账号一槽，保存节点、线索、flags、地图位置。对白从当前句开头恢复，手机保存阅读/删除等 flags，不保存当前打开的页面；步行从该段起点恢复，调查记录保留；弹幕从关卡开头重试。结局到达时写入存档，常规节点仍需手动保存。没有云同步。file: 的存储行为取决于浏览器，HTTP 与 file: 存档互不迁移。
...
- 旧测试只遍历 next / choices，遗漏手机和步行出口；存档测试引用已不存在的 classroom 剧情节点。现在覆盖全部 42 个节点且保留地图规则验证。
...
| 图片集中映射与本地占位 | 已完成 | P0：补齐逐场正式背景、立绘、道具；不能把共用占位当成成品 |
| 音频 | BGM 播放机制已有，正式音频未提供、剧本未逐场配置 BGM | P1：配乐节点、音效、配音播放与音量设置 |
| 42 节点序章、手机、步行、两个结局 | 保留并修复上述问题 | P1：完整人工通关、细化演出与台词停顿 |
| 存档 | v1 单槽本地存档 | P2：多槽、截图、导入导出、自动保存策略；扩展需设计迁移 |
...
2026-09-06：`npm test` 10/10 通过。覆盖剧情可达、地图线索可达、推理门槛、存档往返与坏档拒绝、普通脚本依赖、结局成就兼容、资源登记和文件存在、多人物/CG 与缺图降级契约、手机通讯录不被略过与退出定时器清理，以及全 game 的 Zpix 字体约束。
