...
- 演出：新增 `monologue` 与 `heroine-card` 显示模式。方括号文本使用无对话框的全屏文字；普通台词、心声和旁白继续使用 Galgame 对话框。制作备注不进入玩家文本，其余制作标签保留在节点 `production` / `postProduction` 元数据中。
...
| 女主视角篇 | 已接入 514 个节点，全屏独白、台词、心声、旁白和制作元数据均保留 | P1：按稳定 ID 补齐咖啡店、路人立绘与正式背景 |
