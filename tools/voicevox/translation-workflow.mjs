...
async function readRoleAliases(filename = join(process.cwd(), 'game/data/voice/speaker-role-map.json')) {
  try {
    const value = JSON.parse(await readFile(filename, 'utf8'));
...
export async function exportTranslationBundle(outputDirectory, options = {}) {
  ...
  const aliases = options.aliases || await readRoleAliases(options.roleMapFile);
  ...
  for (const row of rows) {
    const voiceRole = aliases[row.speaker] || row.speaker || '未分配角色';
...
export async function exportApprovedScripts(projectFile, outputDirectory, roleMapFile) {
  ...
  const aliases = await readRoleAliases(roleMapFile);
  const current = new Map(source.lines.map(line => [line.lineId, line]));
  const groups = new Map();
  for (const value of Object.values(translations)) {
    ...
    const voiceRole = aliases[line.speaker] || line.speaker || '未分配角色';
    if (!groups.has(voiceRole)) groups.set(voiceRole, []);
...
export async function exportVoiceRoleBatches(projectFile, outputDirectory, roleMapFile) {
  ...
  const config = roleMapFile ? JSON.parse(await readFile(roleMapFile, 'utf8')) : { aliases: {} };
  const aliases = config.aliases || {};
  const groups = new Map();
  for (const value of Object.values(project.lines || {})) {
    if (!String(value.speechText || '').trim()) continue;
    const voiceRole = aliases[value.speaker] || value.speaker || '未分配角色';
    if (!groups.has(voiceRole)) groups.set(voiceRole, []);
