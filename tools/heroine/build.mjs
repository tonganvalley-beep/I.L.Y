 ...
 * script has its own labelled grammar: production directions are retained as
 * metadata, bracketed prose becomes full-screen narration, and only spoken or
 * narrative lines become Galgame dialogue.
 */
...
for (let index = 0; index < lines.length; index += 1) {
  ...
  const fullScreenMatch = line.match(/^［\s*(.*?)\s*］$/);
  if (fullScreenMatch) {
    addNode(nextId(), 'monologue', fullScreenMatch[1]);
    counts.fullscreen += 1;
    continue;
  }
  ...
  if (innerMatch) {
    const [role] = innerMatch[1].split('·');
    addNode(nextId(), 'dialogue', innerMatch[2], `${role}（心声）`);
    counts.inner += 1;
    continue;
  }

  const dialogueMatch = line.match(/^([^「]{1,40})[　 ]+「(.*)」$/);
  if (dialogueMatch) {
    addNode(nextId(), 'dialogue', dialogueMatch[2], dialogueMatch[1].trim());
    counts.dialogue += 1;
    continue;
  }
...
const dialogueCount = counts.dialogue + counts.inner + counts.narration;
console.log(`heroine: ${sequence.length} nodes (${counts.fullscreen} full-screen, ${dialogueCount} dialogue/narration, ${counts.scene} scenes)`);
