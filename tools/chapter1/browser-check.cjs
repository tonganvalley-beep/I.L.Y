...
(async()=>{
 ...
 await load('ch1_g1',{x:6,y:7});await page.keyboard.press('e');assert.equal((await snapshot()).flags.CLEAN_NUM,1);
 ...
 await load('ch1_g2',{x:15,y:2.4});await page.waitForSelector('.dialogue',{timeout:6000});
