...
# npc=True 让渲染器补接触阴影、并把交互光标抬到头顶而不是压在立绘上。
def npc(e,image): e['image']=image; e['npc']=True; e['visual']=dict(x=round(e['x']-1/6,4),y=round(e['y']-3/8,4),w=round(4/3,4),h=round(4/3,4)); return e
make('ch3-work','工作地点外 · 夜路','night',[
 npc(event('coworker',9,10,'同事',text='今天也辛苦了。早点休息吧。'),'npc-coworker'),
...
for m in list(old.values())+list(maps.values()):
    if m['id']=='ch1-room':continue
    paths[m['id']+'-map']='assets/images/maps/'+m['id']+'.png'
...
paths['ch3-mall-map']='assets/images/maps/ch3-mall.png'
(ROOT/'game/data/map-assets.js').write_text('Object.assign(ILY.data.assets.images, '+json.dumps(paths,ensure_ascii=False,indent=2)+');\n',encoding='utf-8')
# 底图不再是手写 SVG，而是由 tools/maps/render_map_art.py 生成
# （AI 地形底图 × 按 JSON 坐标程序化绘制的物件），改完数据后请一并重跑：
import subprocess,sys
subprocess.run([sys.executable,str(ROOT/'tools/maps/render_map_art.py')],check=False)
