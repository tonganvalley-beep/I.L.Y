"""Shared compiler for the reviewed chapter 2/3/final transcripts.
...
Each chapter owns its source and public build entry point under ``tools/<chapter>/``.
Run this file directly only when all later chapters and their chapter-specific asset
manifests need to be rebuilt together.
"""
import json
import re
...
SOURCE_PATHS = {key: ROOT / 'tools' / key / 'source.txt' for key in TITLES}
ART = {
...
class Chapter:
    def __init__(self, key, source_path=None):
        self.key = key
        self.prefix = {'chapter2':'ch2', 'chapter3':'ch3', 'final':'fin'}[key]
        self.source_path = Path(source_path) if source_path else SOURCE_PATHS[key]
        self.lines = self.source_path.read_text(encoding='utf-8').splitlines()
    ...
    def build(self):
        ...
        write(ROOT / f'game/data/story/{self.key}.js', dict(id=self.key, title=TITLES[self.key], start=f'{self.prefix}_s01', nodes=self.nodes),
              f'// Generated by tools/{self.key}/build.py from tools/{self.key}/source.txt.\nILY.data.stories.{self.key} = ')
...
def build_chapter(key):
    """Build exactly one chapter without touching the other story outputs."""
    if key not in TITLES:
        raise ValueError(f'Unknown chapter: {key}')
    chapter = Chapter(key)
    chapter.build()
    print(key, len(chapter.nodes))
    return chapter


def write_asset_block(path, assets):
    path.write_text('// Generated by tools/chapters/build.py. Do not edit directly.\n'
                    'Object.assign(ILY.data.assets.images, '
                    + json.dumps(assets, ensure_ascii=False, indent=2) + ');\n', encoding='utf-8')


def build_asset_manifests():
    """Rebuild later-chapter asset manifests without mixing their ownership."""
    chapter2_assets={'ch2-'+k:'assets/images/'+('backgrounds' if v[2] == 'background' else 'cg')+'/ch2-'+k+'.jpg' for k,v in ART.items()}
    chapter2_assets.update({'ch2-path-summer':'assets/images/maps/ch2-flowers.svg','ch2-follower':'assets/images/maps/airi-follower.svg','airi-rpg-sheet':'assets/images/maps/airi-rpg-sheet.png'})
    chapter3_assets={'ch3-work':'assets/images/maps/ch3-work.svg','ch3-mall':'assets/images/maps/ch3-mall.svg'}
    write_asset_block(ROOT/'game/data/assets/chapter2.js', chapter2_assets)
    write_asset_block(ROOT/'game/data/assets/chapter3.js', chapter3_assets)
