...
`ch1-room.art.renderer = "classic-room"` 保留原有近景跟随、整数像素镜头与手工图块兜底。正式画面使用 `assets/images/maps/ch1-room.png`，逻辑地图按同一 3:2 比例重做为 24×16 格；书架、电脑桌、浴室门框、床、零食桌、坐垫和玄关均由 `tiles` 中连续的碰撞块贴合边界，中央与家具前方保留可完成全部调查的连续通路。`decor` 同步描述兜底画面，清扫垃圾继续使用事件的 `visual` 矩形，拾取后消失。

镜头按约 11 列×7 行可见区域选择整数像素倍率，保持明显的近景观察范围。镜头在房间内部跟随人物，到地图四边停止滚动，手机同样裁切显示。人物保留原素材及原行走动画，不使用重绘人物、不降采样；人物与场景一起随镜头放大。鼠标/触屏坐标使用相同镜头变换反算。
