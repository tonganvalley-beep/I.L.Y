...
运行 `npm run chapters` 可打开章节索引并免登录试玩第一章。正式流程从序章主线终幕点击「进入第一章」衔接；序章「门内的回信」分支保留原结局，不强行接入海边重逢主线。原开始菜单和账号分区仍有效。
...
- JSON 构建的八张地图：出租屋、便利店、水族馆六区；地图仍由 JSON 构建，人物使用连续浮点坐标、逐帧移动和身体碰撞，取消可见网格。操控角色为成田基生，复用原鼠标光标的正面、背面和左右两帧像素素材。
- G1 三处清扫；G2 浴室门接触；G3 两只手柄与电脑开局；G4 食品选择及结账短对白；G5 目击线索、拍照点与具足虫调查、手机无信号、十屋惨叫、空水槽重逢。
- 两阶段弹幕教学：先完成 45 秒红心练习，再进入蓝心重力与跳跃训练。红心关复用六种随机交叠波次、回血弹、预警激光与影分身；蓝心关依次练习落地跳跃、越过低骨条和按住跳过高骨条，碰撞只重试当前步骤且不扣血。键盘与触屏均可操作，失败或任意阶段都可直接继续剧情。配置位于 `data/battles/first-trial.js`。
...
`ch1-store/street/aquarium-outside/aquarium/panorama/empty-tank` 是背景位；`ch1-cg-blue/mirror/bathroom/collapse/reflection` 是 CG 位；`ch1-airi-casual/toya/rui` 是人物位。`ch1-tile-*`、`ch1-object-*` 当前为空，使用几何占位；填入相对 game/index.html 的真实路径即可换图。基生使用 `ch1-sprite-kio` 和 `ch1-kio-*` 动画 ID。
...
正式美术、BGM/SE/配音、表情差分、镜头动画尚待接入；当前提供可玩的剧情与地图占位版本。第二、三章只保留待续信息，不代表后续章节已经实现。新章中文内容未翻译，原菜单中英文开关仍保留。
