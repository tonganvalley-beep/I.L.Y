...
点击画面或按空格 / Enter：先补全文本，再进入下一句。选项必须明确选择。右上角「回滚」、PageUp 或在对白画面向上滚轮可退回上一段；回滚会同时恢复当时的分支标记、线索、地图位置与玩法进度，读档和重新开始会清空本轮回滚记录。隐藏对白按钮或 H 隐藏文本，再点「显示对白」恢复。右上角菜单提供存读档、音乐开关、原生全屏、调查线索与返回入口。
