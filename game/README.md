...
剧本编辑器当前已发布的全部文本修改保存在 `data/story/script-edits.js`，包括改写、说话人、归类、新增和删除；游戏与编辑器都会加载，空浏览器也能生效。这个文件独立于生成的章节数据，运行 `npm run build:story` 不会覆盖它。

编辑器的“保存修改”仍只保存到本机浏览器。后续要写入项目：点击“导出 JSON”，在仓库根目录运行 `node tools/import-script-review.mjs "导出文件的完整路径"`，核对 Git 差异后提交并推送。导入会检查段落及新增位置；`--check` 只校验。“清除本地修改”恢复项目已发布版本。

游戏唯一入口是仓库根目录 `index.html`，完整流程为根入口 → 登录 / 开场 → 开始菜单 → game。直接打开 `game/index.html` 会自动返回根入口。保持文件夹结构，无需安装依赖。开发预览请在根目录运行 `npm start`，浏览器打开 `http://127.0.0.1:8080/`；`npm test` 运行回归检查。
