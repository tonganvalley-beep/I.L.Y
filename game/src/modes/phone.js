...
function mountPhone({ stage, node, state, assets, go, notify }) {
  ...
  function openMail(id) {
    ...
    if (!flags.phone.read.includes(id)) {
      ...
      if (flags.MAIL_READ_COUNT >= 3) unlock(state, 'daily', '每日一封', notify);
    ...
    const full = useScroll ? [m.body, ...m.scrollReveal.ellipsis] : [m.body];
    if (useScroll) {
      const linkLine = el('div', 'd-link', m.scrollReveal.link);
      linkLine.style.display = 'none';
      wrap.append(linkLine);
      const cap = el('div', 'd-cap', m.scrollReveal.linkLabel);
      cap.style.display = 'none';
      wrap.append(cap);
      mailScroll = { linkLine, cap, reveal: m.scrollReveal, onReveal: cfg.onReveal };
    }
    mailChars = Array.from(full.join('\n'));
  ...
  function afterConfirm(id, didDelete) {
    ...
    // 爱理：必须最后处理
    if (id === 'airi') {
      if ((cfg.requireDeleteCount || 3) > flags.CONTACT_DELETED) {
        notify('先看看其他人吧。'); render(); return;
      }
      if (cfg.allowSend) { openEditor(); return; }
      render(); return;
    }
    // 全部强制联系人删除完毕 → 进入发信或退出
  ...
  function renderAlbum() {
    ...
    ids.forEach((id, i) => {
      ...
      else cell.append(el('div', 'cell-ph', '【照片 ' + id + '】'));
  ...
  function onKey(e) {
    ...
    if (view === 'mail') {
      ...
      if (k === ' ' || k === 'Enter') { scrollStep = 0; render(); e.preventDefault(); return; }
      return;
    }
    if (view === 'contact') {
    ...
    if (view === 'confirm') {
      ...
      if (k === 'ArrowDown') { confirmSel = 1; markConfirm(); e.preventDefault(); return; }
      if (k === ' ' || k === 'Enter') {
    ...
    if (k === ' ' || k === 'Enter') {
      if (tab === 'mail') openMail(mailIds[sel]);
  ...
  function maybeShowExit() {
    if (cfg.manualExit && sceneComplete() && view === 'list') {
