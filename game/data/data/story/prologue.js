...
ILY.data.stories.prologue = {
  ...
  nodes: {
    ...
    s01_phone: { type:"phone", background:"bg-apartment-dusk", phone:{
      // 手机首次登场：跳过图标主页，四封邮件全部列出供玩家翻阅；点到 A04 直接进剧情
      tab:"mail", startView:"mail", lockClose:true, mails:["A01","A02","A03","A04"],
      exitOnMail:"A04", exitNext:"s01_photo"
    } },
    ...
    s03_phone: { type:"phone", background:"bg-university", phone:{
      // 示例部分：基生自己动手删掉便利店同事，玩家按空格逐步观看，箭头指向下一步要点的地方
      tab:"contacts", lockClose:true, manualExit:true, contacts:["work"], tutorialDelete:true,
      autoDeleteDemo:true, mails:["F01"], exitNext:"s03_family"
    } },
    ...
    // ===== 场景07 昏暗幽长的隧道 =====
    // 出门走出房子：先走廊关灯一页，再声控灯亮起一页，然后隧道口画面，随后进入隧道步行
    s07: { type:"dialogue", speaker:"旁白", background:"bg-hallway-dark",
      text:"拉开房门，走廊里一片漆黑。声控灯毫无反应，只有楼道尽头的小窗透进一点微光。——出了这扇门，就是今天了。", next:"s07a" },
    s07a: { type:"corridor", background:"bg-hallway",
      text:"脚步声落下，声控灯随你的脚步明灭。走廊尽头的小窗透进一点微光——那是通往海岸的隧道口。",
      corridor:{ bgOn:"bg-hallway", bgOff:"bg-hallway-dark", steps:3, next:"s07b",
        texts:[
          "一步。声控灯亮起，又在你停步的瞬间熄灭。",
          "两步。惨白的光顺着走廊铺过来，又缩回黑暗。",
          "三步。走廊尽头的小窗近了，海风的味道也近了。"
        ] } },
    s07b: { type:"dialogue", speaker:"旁白", background:"bg-tunnel",
      text:"隧道口就在眼前。入口处只有一盏路灯亮着，再往里，黑暗像水一样浓。墙面贴满褪色的租房广告，一张翘起的纸角在风里反复拍墙。隧道深处隐约传来海浪。", next:"s07c" },
    ...
    s07_walk: { type:"walk", walk:{
      bg:"bg-tunnel", player:"kio-walk", length:1600, exitX:1450, playerStartX:60, speed:170,
      /* 快要走完隧道时（88% 进度）冻结操作，全屏淡入隧道画面（镜像缓推 + 暗角），停留后走出 */
      reveal:{ atPct:0.88, hold:2400, flip:true },
      hotspots:[
        { id:"vending", x:380, label:"故障自动贩卖机", text:"……还在放着十年前的广告啊。所有商品都显示“售罄”。" },
        { id:"poster", x:800, label:"褪色海岸海报", text:"海报上的蓝天白沙，与隧道里的黑暗形成刺眼的对比。" },
        { id:"crack", x:1180, label:"隧道墙缝", text:"海风从裂缝穿过，发出很细的哨音。……快到了。" }
      ], exitNext:"s07_exit"
    } },
