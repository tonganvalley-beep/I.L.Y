...
ILY.data.stories.prologue = {
  ...
  nodes: {
    ...
    s02b: {
      type: "dialogue", speaker: "主管", background: "bg-apartment-dusk",
    ...
    // ===== 场景03 回忆 灰色的十年 =====
    s03a: {
      type: "dialogue", speaker: "", background: "bg-university",
    ...
    s03_phone: {
      type: "phone", background: "bg-university", phone: {
    ...
    s03_family: {
      type: "dialogue", speaker: "", background: "bg-apartment-dusk",
      text: "对我来说早已不存在什么能称之为朋友的人了。连家人也早就……进了大学以后，爸爸和妈妈再也没有联系过我。", next: "s03_bankbook"
    },
    s03_bankbook: {
      type: "dialogue", speaker: "旁白", background: "bg-apartment-dusk", gallery: "bankbook",
      text: "一本银行存折上，大学入学后每月固定入账60,000日元，备注栏一片空白。", next: "s03_family2"
    },
    s03_family2: {
      type: "dialogue", speaker: "成田基生", background: "bg-apartment-dusk",
    ...
    s04_empty: {
      type: "dialogue", speaker: "", background: "bg-empty-apartment",
    ...
    s05e: {
      type: "dialogue", speaker: "旁白", overlay: "parcel-label", background: "bg-apartment-dusk",
    ...
    s05g: {
      type: "dialogue", speaker: "快递员", background: "bg-apartment-dusk",
    ...
    s05j: {
      type: "dialogue", speaker: "旁白", background: "bg-apartment-night",
      text: "翻盖手机的屏幕从幽绿变成清澈的蓝光。整个房间的色调在一秒内从灰橙变为深蓝，连基生脸上的阴影都变成了海的颜色。", next: "s05k"
    },
    s05k: { type: "dialogue", speaker: "", background: "bg-apartment-night", text: "海浪的铃声，蓝色的光……", next: "s05l" },
    s05l: { type: "dialogue", speaker: "", background: "bg-apartment-night", text: "那是只属于爱理的来电设置。", next: "s05m" },
    s05m: { type: "dialogue", speaker: "旁白", background: "bg-apartment-night", text: "他手微微发颤，点开了那封回信。", next: "s06a" },

    // ===== 场景06 爱理的回信 =====
    s06a: {
      type: "phone", background: "bg-apartment-night", phone: {
    ...
    s06_react2: {
      type: "dialogue", speaker: "", background: "bg-apartment-night",
    ...
    s06_white_kio1: { type: "dialogue", speaker: "", background: "bg-room-white", text: "也许我终于疯了吧。", next: "s06_white_kio2" },
    s06_white_kio2: {
      type: "dialogue", speaker: "", background: "bg-room-white",
    ...
    s06_break2: {
      type: "dialogue", speaker: "成田基生", background: "bg-apartment-night",
      text: "（崩溃中自嘲）哈哈……把这些、全部都扔了吧，手机也好、其他也好……", next: "br01"
    },
    br01: {
      type: "choice", speaker: "成田基生", background: "bg-apartment-night", text: "把手机……",
      choices: [
    ...
    s07: {
      type: "corridor", background: "bg-hallway-dark",
      text: "出租屋外，走廊一片漆黑。按 空格 / Enter 或点击画面，一步步往楼道口走——声控灯会随脚步亮起又熄灭。",
      corridor: {
        bgOn: "bg-hallway", bgOff: "bg-hallway-dark", steps: 3, next: "s07a",
        texts: [
          "第一步落下，最近的声控灯亮起，又灭。",
          "第二步，光晕向隧道口推近了一点。",
          "第三步，隧道口的轮廓在黑暗里浮现。"
    ...
    s07a: {
      type: "dialogue", speaker: "旁白", background: "bg-hallway",
      text: "脚步声惊动了声控灯，一盏接一盏亮起又熄灭。通往附近海岸，要穿过一条昏暗幽长的隧道。", next: "s07b"
    },
    s07b: {
      type: "dialogue", speaker: "旁白", background: "bg-tunnel",
      text: "入口处只有一盏路灯亮着，再往里，黑暗像水一样浓。墙面贴满褪色的租房广告，一张翘起的纸角在风里反复拍墙。隧道深处隐约传来海浪。", next: "s07c"
    },
    s07c: {
      type: "dialogue", speaker: "", background: "bg-black",
      text: "穿过这个隧道，就能到附近的海岸。……什么都不会有的。谁也不会来。", next: "s07_walk"
    },
    s07_walk: {
      type: "walk", walk: {
        bg: "bg-black", player: "kio-walk", length: 2400, exitX: 2200, playerStartX: 60, speed: 170,
        reveal: { bg: "bg-tunnel", atPct: 0.88, hold: 2400, flip: true },
        hotspots: [
    ...
    s07_exit: {
      type: "dialogue", speaker: "旁白", background: "bg-coast-night",
      text: "走出隧道出口的一瞬间，风忽然变大，带着咸味。月光下的海岸在眼前铺开。云层很薄，月光落在海面上碎成一条长长的银路；海浪缓缓涌上沙滩，又缓缓退去。", next: "s08a"
    },

    // ===== 场景08 无边的海岸 =====
    s08a: {
      type: "dialogue", speaker: "成田基生", background: "bg-coast-night",
    ...
    // ===== 场景09 邮件的下面 =====
    s09: {
      type: "phone", background: "bg-coast-night", phone: {
    ...
    br02: {
      type: "choice", speaker: "成田基生", background: "bg-coast-night",
    ...
    // ===== 场景11 十年前的她 =====
    s11a: {
      type: "dialogue", speaker: "旁白", background: "bg-coast-blue",
      text: "无边海岸的蓝光中央站着一个人影——穿校服的短发少女。深蓝短发被无形光流轻轻托起，蔚蓝发卡在额侧闪着微光，水手服裙摆静止在无风的空气里，像一张从十年前剪下的照片，被贴进这个夜晚。", next: "s11a2"
    },
    s11a2: { type: "dialogue", speaker: "成田基生", background: "bg-coast-blue", text: "……爱理……？", next: "s11_turn" },
    s11_turn: {
      type: "dialogue", speaker: "旁白", background: "bg-coast-blue",
      text: "少女缓缓转过头，睫毛轻抬。她睁着蓝色的双眸，看向基生，露出与十年前一模一样、自然又明亮的微笑。", next: "s11b"
    },
    s11b: {
      type: "dialogue", speaker: "", background: "bg-coast-blue",
      text: "怎么可能……爱理……为什么？这不可能……爱理？不可能会是她……这不可能……不可能是爱理……这不可能……不可能……", next: "s11b2"
    },
    s11b2: { type: "dialogue", speaker: "", background: "bg-coast-blue", text: "（抓住唯一的理由，声音发颤）因为、因为爱理她——", next: "s11b3" },
    s11b3: { type: "dialogue", speaker: "", background: "bg-coast-blue", text: "（几乎耳语）和我一样大啊。", next: "s11d" },
    s11c: { type: "dialogue", speaker: "字幕", background: "bg-room-white", text: "十年后的爱理，本该和他一样28岁了。可是眼前的少女，还穿着那身校服。", next: "s11d" },
    s11d: {
      type: "dialogue", speaker: "百合沢爱理", portrait: "portrait-airi", background: "bg-coast-blue",
      text: "她微笑着朝基生伸出双手，掌心向上，既像邀请，也像等待被握住。（轻快，温柔，尾音上扬，与记忆分毫不差）最喜欢你了，基生。", next: "s11d2"
    },
    s11d2: { type: "dialogue", speaker: "成田基生", background: "bg-coast-blue", text: "（失神，几乎听不见）……爱理。", next: "ch1_s01" },
    s11e: {
      type: "dialogue", speaker: "字幕", background: "bg-coast-blue",
      text: "蓝光涨满整个屏幕。标题 I.L.Y. 浮现，三个字母像被海水冲刷过的沙字，先被浪抹平，又重新浮现。——在那里我遇到的，是十年前的那个她。", next: "finale"
    },

    // ===== 终幕 / 分支结局 =====
    finale: {
      type: "finale", background: "bg-coast-blue", title: "I.L.Y.", subtitle: "——在那里我遇到的，是十年前的那个她。",
