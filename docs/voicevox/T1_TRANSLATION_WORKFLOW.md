...
输出位于 `game/data/voice/voicevox-import/`，每个配音角色一份 TSV，可在项目配音编辑器中逐个批量导入并统一设置 VOICEVOX 风格。角色归并由 `game/data/voice/speaker-role-map.json` 显式控制：“成田基生”和“基生”归为同一角色；用户于 2026-09-17 确认“爱理”、带引号的“爱理”和“百合沢爱理”（亦写作“百合尺爱理”）共用同一声线，统一导出为 `百合沢爱理.tsv`，原始标签仍保留在 `source_speaker` 列。“成年爱理”和“成年‘爱理’”暂保留独立映射。
